...
import { clampInt, matrixMultiply } from './math';
...
export function argbFromLinrgb(linrgb: number[]): number {
	const r = delinearized(linrgb[0] ?? 0);
	const g = delinearized(linrgb[1] ?? 0);
	const b = delinearized(linrgb[2] ?? 0);
...
export function delinearized(rgbComponent: number): number {
	...
	let delinearized = 1.055 * Math.pow(normalized, 1.0 / 2.4) - 0.055;
