...
import { blueFromArgb, greenFromArgb, redFromArgb } from '../utils';

import { QuantizerMap } from './quantizer-map';
...
export class QuantizerWu {
	...
	private createBoxes(maxColors: number): CreateBoxesResult {
		...
		for (let i = 1; i < maxColors; i++) {
			...
			next = 0;
			let temp = volumeVariance[0] ?? 1;
			for (let j = 1; j <= i; j++) {
				if ((volumeVariance[j] ?? 0) > temp) {
					temp = volumeVariance[j] ?? 1;
					next = j;
				}
			}
			if (temp <= 0.0) {
	...
	private top(cube: Box, direction: string, position: number, moment: number[]) {
		switch (direction) {
			case directions.RED:
				return (
