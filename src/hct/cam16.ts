...
import { argbFromXyz, linearized, sanitizeDegreesDouble, signum } from '../utils';
...
export class Cam16 {
	...
	static fromIntInViewingConditions(argb: number, viewingConditions: ViewingConditions): Cam16 {
		...
		const rD = (viewingConditions.rgbD[0] ?? 1) * rC;
		const gD = (viewingConditions.rgbD[1] ?? 1) * gC;
		const bD = (viewingConditions.rgbD[2] ?? 1) * bC;
		...
		const j =
			100.0 * (ac / viewingConditions.aw) ** (viewingConditions.c * viewingConditions.z);
		const q =
	...
	viewed(viewingConditions: ViewingConditions): number {
		...
		const t = (alpha / (1.64 - 0.29 ** viewingConditions.n) ** 0.73) ** (1.0 / 0.9);
		...
		const rC = signum(rA) * (100.0 / viewingConditions.fl) * rCBase ** (1.0 / 0.42);
		const gCBase = Math.max(0, (27.13 * Math.abs(gA)) / (400.0 - Math.abs(gA)));
		const gC = signum(gA) * (100.0 / viewingConditions.fl) * gCBase ** (1.0 / 0.42);
		const bCBase = Math.max(0, (27.13 * Math.abs(bA)) / (400.0 - Math.abs(bA)));
		const bC = signum(bA) * (100.0 / viewingConditions.fl) * bCBase ** (1.0 / 0.42);
		const rF = rC / (viewingConditions.rgbD[0] ?? 1);
		const gF = gC / (viewingConditions.rgbD[1] ?? 1);
		const bF = bC / (viewingConditions.rgbD[2] ?? 1);
