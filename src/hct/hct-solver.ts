...
export const HctSolver = {
	...
	CRITICAL_PLANES: [
		...
		92.58406282226491, 93.43925555268066, 94.29903859396902, 95.16341895893969,
	 ...
	 */
	sanitizeRadians(angle: number): number {
	 ...
	 * regular RGB space
	 */
	trueDelinearized(rgbComponent: number): number {
		const normalized = rgbComponent / 100.0;
		let delinearized = 1.055 * normalized ** (1.0 / 2.4) - 0.055;
	...
	chromaticAdaptation(component: number): number {
	 ...
	 */
	hueOf(linrgb: number[]): number {
		const scaledDiscount = matrixMultiply(linrgb, HctSolver.SCALED_DISCOUNT_FROM_LINRGB);
		const rA = HctSolver.chromaticAdaptation(scaledDiscount[0] ?? 0);
		const gA = HctSolver.chromaticAdaptation(scaledDiscount[1] ?? 0);
		const bA = HctSolver.chromaticAdaptation(scaledDiscount[2] ?? 0);
		// redness-greenness
	...
	areInCyclicOrder(a: number, b: number, c: number): boolean {
	 ...
	 */
	intercept(source: number, mid: number, target: number): number {
		return (mid - source) / (target - source);
	},

	lerpPoint(source: number[], t: number, target: number[]): number[] {
		return [
			(source[0] ?? 1) + ((target[0] ?? 0) - (source[0] ?? 0)) * t,
			(source[1] ?? 1) + ((target[1] ?? 0) - (source[1] ?? 0)) * t,
			(source[2] ?? 1) + ((target[2] ?? 0) - (source[2] ?? 0)) * t
		];
	},
	 ...
	 */
	setCoordinate(source: number[], coordinate: number, target: number[], axis: number): number[] {
		const t = HctSolver.intercept(source[axis] ?? 0, coordinate, target[axis] ?? 0);
	 ...
	 */
	nthVertex(y: number, n: number): number[] {
		const kR = HctSolver.Y_FROM_LINRGB[0] ?? 1;
		const kG = HctSolver.Y_FROM_LINRGB[1] ?? 1;
		const kB = HctSolver.Y_FROM_LINRGB[2] ?? 1;
	 ...
	 * desired color.
	 */
	bisectToSegment(y: number, targetHue: number): number[][] {
	...
	criticalPlaneBelow(x: number): number {
		return Math.floor(x - 0.5);
	},

	criticalPlaneAbove(x: number): number {
	 ...
	 */
	bisectToLimit(y: number, targetHue: number): number[] {
		const segment = HctSolver.bisectToSegment(y, targetHue);
		let left = segment[0] ?? [0];
		let leftHue = HctSolver.hueOf(left);
		let right = segment[1] ?? [0];
		for (let axis = 0; axis < 3; axis++) {
			if (left[axis] !== right[axis]) {
				let lPlane = HctSolver.criticalPlaneAbove(
					HctSolver.trueDelinearized(left[axis] ?? 1)
				);
				let rPlane = HctSolver.criticalPlaneBelow(
					HctSolver.trueDelinearized(right[axis] ?? 1)
				);
				...
				for (let i = 0; i < 8; i++) {
					if (Math.abs(rPlane - lPlane) <= 1) {
						...
						const midPlaneCoordinate = HctSolver.CRITICAL_PLANES[mPlane] ?? 1;
	...
	inverseChromaticAdaptation(adapted: number): number {
	 ...
	 * otherwise.
	 */
	findResultByJ(hueRadians: number, chroma: number, y: number): number {
		...
		for (let iterationRound = 0; iterationRound < 5; iterationRound++) {
			...
			// Operations inlined from Cam16 to avoid repeated calculation
			// ===========================================================
			if ((linrgb[0] ?? 1) < 0 || (linrgb[1] ?? 1) < 0 || (linrgb[2] ?? 1) < 0) {
				return 0;
			}
			const kR = HctSolver.Y_FROM_LINRGB[0] ?? 1;
			const kG = HctSolver.Y_FROM_LINRGB[1] ?? 1;
			const kB = HctSolver.Y_FROM_LINRGB[2] ?? 1;
			const fnj = kR * (linrgb[0] ?? 1) + kG * (linrgb[1] ?? 1) + kB * (linrgb[2] ?? 1);
			if (fnj <= 0) {
				return 0;
			}
	 ...
	 */
	solveToInt(hueDegrees: number, chroma: number, lstar: number): number {
	 ...
	 * sufficiently close, and chroma will be maximized.
	 */
	solveToCam(hueDegrees: number, chroma: number, lstar: number): Cam16 {
