...
import { Hct } from '../hct';
import { differenceDegrees, sanitizeDegreesInt } from '../utils';
...
export const Score = {
	 ...
	 */
	score(colorsToPopulation: Map<number, number>, options?: ScoreOptions): number[] {
		...
		const scoredHct: { hct: Hct; score: number }[] = [];
		for (const hct of colorsHct) {
			const hue = sanitizeDegreesInt(Math.round(hct.hue));
			const proportion = hueExcitedProportions[hue] ?? 0;
			if (
				filter &&
				(hct.chroma < Score.CUTOFF_CHROMA || proportion <= Score.CUTOFF_EXCITED_PROPORTION)
			) {
				continue;
			}
		...
		for (let difference = 90; difference >= 15; difference--) {
			...
			for (const { hct } of scoredHct) {
				const duplicateHue = chosenColors.find((chosenHct) => {
					return differenceDegrees(hct.hue, chosenHct.hue) < difference;
				});
				if (!duplicateHue) {
